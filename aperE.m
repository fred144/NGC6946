...
err=sqrt(abs(sum(sig).^2+ssig^2));                 % total error
issat=0;
...
properties = imagesc(px(1,:),py(:,1),pz); 
%properties = imagesc(px(1,:),py(:,1),[0 1]);
properties.CData;
%properties.CDataMapping = 'direct'; 
...
yc=sin(p);
hold on
plot(col+rad1*xc,row+rad2*yc,'green');
plot(col+ir1*xc,row+ir2*yc,'w');
plot(col+or1*xc,row+or2*yc,'w');
if (issat),
